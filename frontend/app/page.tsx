      ...
      <section className="relative mx-auto max-w-6xl px-6 pb-16">
        <div className="grid gap-6 md:grid-cols-3">
          {[
            ...
            { title: 'Beneficios', text: 'Resultados y mejoras para tu equipo.', href: '/beneficios' },
            { title: 'Planes', text: 'Compara planes y elige el ideal.', href: '/planes' },
          ].map((item) => (
            <Link
              key={item.title}
              href={item.href}
              className="rounded-3xl border border-slate-100 bg-white p-6 shadow-sm transition hover:-translate-y-1"
            >
              <p className="text-sm font-semibold text-slate-900">{item.title}</p>
              <p className="mt-2 text-sm text-slate-500">{item.text}</p>
              <span className="mt-4 inline-flex text-xs font-semibold text-[var(--af-primary)]">Ver más</span>
            </Link>
          ))}
        </div>
      </section>
