...
export function PublicReservas({ initialBusinessId }: { initialBusinessId?: string }) {
  const queryClient = useQueryClient();
  const searchParams = useSearchParams();
  const businessIdFromQuery = searchParams?.get('businessId') ?? searchParams?.get('business') ?? '';
  const defaultBusinessId = initialBusinessId || businessIdFromQuery || process.env.NEXT_PUBLIC_DEFAULT_BUSINESS_ID || '';
  ...
  useEffect(() => {
    ...
    setForm((prev) => (prev.businessId === defaultBusinessId ? prev : { ...prev, businessId: defaultBusinessId }));
  }, [defaultBusinessId]);
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_10%_10%,#fef9c3_0%,#f8fafc_35%,#ffffff_100%)] p-4">
      <div className="mx-auto max-w-5xl space-y-6 py-6 md:py-8">
        ...
        {!form.businessId ? (
          <Card className="space-y-2 border-amber-200 bg-amber-50 text-amber-900">
            <p className="text-sm font-semibold">No se encontró el negocio.</p>
            <p className="text-sm">
              Revisa el enlace compartido por el negocio o solicita un link de reservas válido.
            </p>
          </Card>
        ) : null}
            ...
            {(professionalsQuery.data ?? []).map((professional) => {
              ...
              return (
                ...
                >
                  ...
                  <p className={cn('font-medium', isActive ? 'text-white' : 'text-zinc-900')}>
            ...
            {slotsForSelectedDay.length === 0 ? (
              ...
              slotsForSelectedDay.map((slot) => (
                ...
                >
                  ...
                  <p
                    className={cn('text-xs', selectedSlot?.startsAt === slot.startsAt ? 'text-zinc-200' : 'text-zinc-500')}
                  >
